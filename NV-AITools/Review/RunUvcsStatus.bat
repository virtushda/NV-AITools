...
call :SetTimestamp
set "TEMP_OUTPUT=%TEMP%\NV-AITools-status-%RANDOM%-%RANDOM%.tmp"
set "FINAL_OUTPUT=%REVIEWS_DIR%\uvcs-status-%STAMP%.json"

"%NV_AI_TOOLS_EXE%" status --workspace "%WORKSPACE%" > "%TEMP_OUTPUT%"
...
:InvalidOutput
echo ERROR: NV-AITools did not produce valid status JSON.
