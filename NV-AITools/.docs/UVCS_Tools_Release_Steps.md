...
3. Keep the application version in `major.minor.patch` form using non-negative integers, such as `1.2.0`.
...
```bat
Packaging\BuildRelease.bat 1.2.0
```
...
```text
Artifacts\NV-AITools-1.2.0-win-x64.zip
```
...
3. On a Windows account with Plastic SCM and Git installed, run `Install.bat` without elevation.
...
13. Run pending and changeset comparisons with at least 32 eligible text files. Confirm each command reaches but never exceeds 16 active Plastic operations and 16 active local comparisons, begins comparison before all exports finish, and preserves deterministic output order.
